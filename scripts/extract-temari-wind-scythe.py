#!/usr/bin/env python3
"""Split Temari's embedded Wind Scythe art from her Right Kick poses."""
...
PROJECTILE_ID = "wind-scythe-projectile"
SOURCE_FRAME_INDICES = [136, 139, 140, 141, 142, 146, 149, 150]
ALPHA_THRESHOLD = 24
...
def main() -> None:
    ...
    for output_index, frame_index in enumerate(SOURCE_FRAME_INDICES):
        ...
        if not entry or not isinstance(entry.get("box"), list) or len(entry["box"]) != 4:
            raise RuntimeError(f"Missing source box for Temari frame {frame_index}")
        ...
        if len(components) < 2:
            raise RuntimeError(f"Temari frame {frame_index} has no detachable wind components")
        ...
        source_entries.append({
            "index": frame_index,
            "path": f"/characters/temari/frames/frame-{frame_index:03d}.png",
            ...
            "projectileOutput": f"/characters/temari/projectiles/{PROJECTILE_ID}/frames/frame-{output_index:03d}.png",
        })
    ...
    source_metadata = {
        ...
        "source": "Temari original sprite sheet",
        "sourceSheetPath": "/characters/temari/animation-sheet.png",
        "extraction": "largest alpha component retained as fighter and fan; detached alpha components extracted as Wind Scythe",
        "sourceFrames": source_entries,
        "matchedMoves": ["kickright"],
    }
