...
const outPath = path.join(repoRoot, 'tmp/voxel-scale-editor/family-passes/sprint-height-visual-repair.json');
...
for (const entry of entries) {
  ...
  if (character.unplayable || !character.animationFrames?.idle?.length || !character.animationFrames?.sprint?.length) continue;
  ...
  let dirty = false;
  for (const framePath of character.animationFrames.sprint) {
    const frame = frameIndexFromPath(framePath);
    if (!Number.isFinite(frame)) continue;
    const bounds = await voxelBounds(id, frame);
    const scale = animationScaleFor(character, 'sprint', frame);
    ...
    const horizontalSprint = widthRatio > 1.25;
    const tooLow = horizontalSprint && heightRatio < 0.74;
    const tooWide = widthRatio > 2.05;
    if (!tooLow && !tooWide) continue;

    const targetHeight = tooLow ? (widthRatio > 1.7 ? 0.8 : 0.86) : heightRatio;
    const targetWidth = tooWide ? 1.9 : widthRatio;
    ...
    character.animationFrameScales.sprint ??= {};
    character.animationFrameScales.sprint[String(frame)] = { width: nextWidth, height: nextHeight, offsetX: scale.offsetX };
    dirty = true;
    changes.push({
      id,
      key: 'sprint',
      frame,
      reason: [tooLow ? 'horizontal-sprint-too-low-vs-idle-ghost' : null, tooWide ? 'horizontal-sprint-too-wide-vs-idle-ghost' : null].filter(Boolean).join(','),
      ratioBefore: { width: round(widthRatio), height: round(heightRatio) },
      ratioTarget: { width: round(targetWidth), height: round(targetHeight) },
