...
export default function App() {
  ...
  const startInfiniteTournamentMatch = useCallback((bracket: TournamentBracket) => {
    ...
    const characterA = roster.find((character) => character.id === entryA?.characterId);
    const characterB = roster.find((character) => character.id === entryB?.characterId);
    const fightStage = resolveRandomStageSelection();
    ...
    setP1Id(characterA.id);
    setP2Id(characterB.id);
    setLocalTournamentBracket(bracket);
    setOnlineTournamentStatus(null);
    setActiveTournamentMatchId(match.id);
    setTournamentStatusText(`${getTournamentRoundLabel(match.round, getTournamentTotalRounds(bracket))}: ${entryA.displayName} vs ${entryB.displayName}`);
    setMode('tournamentInfinite');
    setScreen('tournamentBracket');
    return true;
  }, [resolveRandomStageSelection, roster]);

  const makeInfiniteTournamentBracket = useCallback(() => {
    return createInfiniteTournamentBracket(
      roster
        .filter((character) => isCharacterUnlocked(character, effectiveUnlockedCharacterIds))
        .map((character) => ({ id: character.id, displayName: character.displayName }))
    );
  ...
  const startInfiniteTournament = useCallback(() => {
    if (!isDevHost) return;
    window.clearTimeout(infiniteTournamentRestartTimerRef.current);
    const bracket = makeInfiniteTournamentBracket();
    if (startInfiniteTournamentMatch(bracket)) {
      captureAppAnalytics('tournament_entry_succeeded', {
        tournament_mode: 'infinite',
        tournament_id: bracket.id
      });
    }
  }, [captureAppAnalytics, isDevHost, makeInfiniteTournamentBracket, startInfiniteTournamentMatch]);

  const clearInfiniteTournamentRestart = useCallback(() => {
    window.clearTimeout(infiniteTournamentRestartTimerRef.current);
  }, []);
  ...
  const handleAppMenuKeyDown = useCallback((event: ReactKeyboardEvent<HTMLElement>) => {
    if (handleMenuNavigationKeyEvent(event.nativeEvent, screen)) {
	  ...
	  const makeArcadeMiniGameLaunch = useCallback((level = arcadeRun.level): ArcadeMiniGameLaunch | null => {
	    ...
	    return {
	      kind: pickArcadeMiniGameKind(level),
	      stage: miniGameStage,
	      seed: freshAiSeed(),
	      arcadeLevel: level
	    };
	  ...
	  useEffect(() => {
	    ...
	    setActiveArcadeMiniGame({
	      ...
	      arcadeLevel: arcadeRun.level
	    });
	    setLastMiniGameResult(null);
	    setScreen('miniGame');
	  }, [arcadeRun.level, p1, playableStageRoster, screen, selectedStage]);
      ...
      <section className="screen-panel">
        ...
        {screen === 'tournamentBracket' && (
          <TournamentBracketIntroScreen
            ...
            matchId={activeTournamentMatchId}
            localEntryId={mode === 'tournamentOnline' ? onlineTournamentStatus?.entry?.id : mode === 'tournamentInfinite' ? undefined : 'local-player'}
            roster={roster}
    ...
    <div className="select-screen versus-select-screen tournament-select-screen">
      ...
      <section className="versus-roster-panel tournament-roster-panel" aria-label="Tournament character select">
        <div className="versus-select-top">
          ...
          <div className="mode-stack">
            <TournamentModeCarousel value={tournamentMode} setValue={selectTournamentMode} isDevHost={isDevHost} />
          </div>
        </div>
      ...
      <section className="versus-hero versus-hero-right tournament-preview-hero" aria-label="Tournament preview">
        ...
        <span className="versus-hero-name">{tournamentMode === 'free' ? 'FREE' : tournamentMode === 'paid' ? 'PAID' : tournamentMode === 'infinite' ? 'INFINITE' : 'ONLINE'}</span>
      </section>
      <div className="versus-floor-glow" aria-hidden="true" />
    </div>
  );
      ...
      <div className="frame-data-grid">
        ...
        <div className="frame-toggle-group" aria-label="Move options">
          ...
          <div>
            ...
            <label className="frame-toggle">
              <span>Holdable</span>
              <input type="checkbox" checked={Boolean(move.holdable)} onChange={(event) => onChange({ holdable: event.target.checked })} />
            </label>
            <label className="frame-toggle">
    ...
    const tick = (now: number) => {
      ...
      if (!pausedRef.current && !completedRef.current) {
        ...
        if (snapshotRef.current.phase === 'complete') {
          completedRef.current = true;
          const previousHighScore = readMiniGameHighScore({ gameId: snapshotRef.current.gameId, stageId: snapshotRef.current.stage.id });
          const result = makeEnemyRushMiniGameResult(snapshotRef.current, previousHighScore);
          const highScore = writeMiniGameHighScore({ gameId: result.gameId, stageId: result.stageId }, result.score);
          onAnalytics('minigame_completed', {
            game_id: result.gameId,
            stage_id: result.stageId,
  ...
  return (
    ...
    >
      ...
      <section className="mini-game-result-copy" aria-live="polite">
        ...
        {typeof result.arcadeScoreAward === 'number' && (
          <b className="mini-game-result-arcade-award">Arcade Score +{Math.round(result.arcadeScoreAward)}</b>
        )}
        {ready && <small>Press any key to continue</small>}
      </section>
    </div>
  ...
  return (
    <div className="mini-game-result-screen arcade-game-over-screen" aria-label="Arcade game over">
      ...
      <section className="mini-game-result-copy arcade-game-over-copy">
        ...
        <h1>{run.score}</h1>
        <p>{run.score >= bestScore ? 'New Best Score' : 'Game Over'}</p>
        <div className="mini-game-result-stats arcade-game-over-stats">
        ...
        <div className="arcade-game-over-details">
        ...
        <div className="overlay-actions pause-menu-actions arcade-game-over-actions">
          ...
          <button className="secondary-button" onClick={onGiveUp}>
            <Home size={18} />
            Give Up
          </button>
        </div>
