...
describe('Story party protocol', () => {
  it('validates membership and rejects stale intent sequences', () => {
    const base = { version: 3, type: 'intent', partyId: 'party-a', senderSessionId: 'guest', sequence: 4, clientTime: 1_000, moveX: 4, moveY: -4, jump: true, block: false, attack: 'jab' };
  ...
  it('accepts only ordered snapshots from the current authority', () => {
    const snapshot = { version: 3, type: 'snapshot', partyId: 'party-a', leaderSessionId: 'leader', authorityEpoch: 2, sequence: 8, serverTime: 1_000, roomId: 'surface', runSeed: null, floorNumber: 0, pressureClockSeconds: 0, eventState: null, boonStacks: {}, ledgerBankEventId: null, endReason: null, actors: [], enemies: [], projectiles: [], encounterState: null, rewardsPaused: false, rewardEvents: [] };
  ...
  it('deduplicates recipient rewards and deterministically targets the nearest enemy', () => {
    ...
    const snapshot = { version: 3, type: 'snapshot', partyId: 'party-a', leaderSessionId: 'leader', authorityEpoch: 1, sequence: 1, serverTime: 1, roomId: 'surface', runSeed: null, floorNumber: 0, pressureClockSeconds: 0, eventState: null, boonStacks: {}, ledgerBankEventId: null, endReason: null, actors: [], enemies: [], projectiles: [], encounterState: null, rewardsPaused: false, rewardEvents: [{ id: 'reward-1', spawnId: 'enemy-1', enemyId: 'ember-fist', tier: 'challenger', xp: 50, recipients: ['leader', 'guest'] }] } satisfies StoryPartyAuthoritativeSnapshot;
