...
function region(input: RegionInput): StoryHubDefinition {
  ...
  const enemySpawns = Array.from({ length: 10 }, (_, index): StoryEnemySpawnDefinition => {
    ...
    return {
      ...
      name: elite ? `${REGION_LABELS[input.id].split(' ')[0]} Colossus` : `${REGION_LABELS[input.id].split(' ')[0]} ${archetype === 'ground' ? 'Stalker' : archetype === 'flying' ? 'Wisp' : 'Caster'}`,
      archetype: elite ? 'ground' : archetype,
      position: [enemyX, !elite && archetype === 'flying' ? 3.2 + (index % 2) * 0.7 : STORY_GROUNDED_ACTOR_CENTER_Y],
      patrolRadius: elite ? Math.min(11, Math.max(5, (encounter.range[1] - encounter.range[0]) / 2 - 2)) : 2.2 + (index % 3) * 0.45,
      sprite: input.sprites[index % input.sprites.length],
      accent: REGION_ACCENTS[input.id],
      encounterZoneId: encounter.id,
      ...(elite ? { elite: true, scale: 2.35, leash: encounter.range as [number, number] } : {})
    };
  });
...
export const STORY_ADVENTURE_WORLDS: Record<StoryAdventureWorldId, StoryHubDefinition> = {
  ...
  greenhollow: region({
    id: 'greenhollow', subtitle: 'A lively settlement at the edge of the old forest', theme: 'village', width: 560, sprites: ['orc', 'slime', 'orc-shaman'],
  ...
  thornwood: region({
    id: 'thornwood', subtitle: 'Ancient roots hide restless creatures', theme: 'forest', width: 600, sprites: ['slime', 'demon', 'reptile'],
  ...
  ironroot: region({
    id: 'ironroot', subtitle: 'Ore-lit tunnels beneath the world route', theme: 'mine', width: 640, sprites: ['orc', 'elemental', 'orc-shaman'],
  ...
  bonevault: region({
    id: 'bonevault', subtitle: 'A sealed crypt filled with roaming skeleton crews', theme: 'crypt', width: 520, sprites: ['skeleton', 'demon', 'skeleton-mage'],
  ...
  emberdeep: region({
    id: 'emberdeep', subtitle: 'A volcanic underworld of fire and hidden relics', theme: 'underworld', width: 620, sprites: ['demon', 'elemental', 'reptile'],
  ...
  frostpeak: region({
    id: 'frostpeak', subtitle: 'A frozen climb through bright mountain winds', theme: 'snow', width: 560, sprites: ['elemental', 'demon', 'skeleton-mage'],
  ...
  sunscar: region({
    id: 'sunscar', subtitle: 'Shifting dunes surrounding a buried road', theme: 'desert', width: 640, sprites: ['reptile', 'orc', 'orc-shaman'],
  ...
  skyglass: region({
    id: 'skyglass', subtitle: 'Floating platforms and unstable ancient traps', theme: 'ruins', width: 480, sprites: ['slime', 'demon', 'elemental'],
