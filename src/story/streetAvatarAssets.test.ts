...
import { STORY_SPRITE_MANIFEST, validateStorySpriteManifest } from './streetAvatarCatalog';
...
function inspectFrame(publicPath: string) {
  ...
  expect(bytes.readUInt32BE(16)).toBe(224);
...
describe('K.O.R.E. full-frame street avatar assets', () => {
  it('contains the four supplied and ten generated aligned motion sets', () => {
    ...
    expect(uniquePaths.size).toBe(520);
  ...
  it('exports a strict binary transparent matte with transparent canvas corners', () => {
    const validation = spawnSync('python3', ['-c', [
      ...
      "assert len(paths)==520, len(paths)",
      'for path in paths:',
      " im=Image.open(path).convert('RGBA')",
      ' alpha=im.getchannel("A")',
      ' assert set(alpha.getdata()) <= {0,255}, path',
      ' assert all(im.getpixel(point)[3] == 0 for point in ((0,0),(223,0),(0,191),(223,191))), path',
      ' pixels=alpha.load()',
      ' pinholes=[(x,y) for y in range(1,191) for x in range(1,223) if pixels[x,y] == 0 and all(pixels[nx,ny] for nx,ny in ((x-1,y),(x+1,y),(x,y-1),(x,y+1)))]',
