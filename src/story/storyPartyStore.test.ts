...
describe('Adventure party registry v3', () => {
  it('uses generation v3 and fills leader capacity with deterministic AI', () => {
    const party = normalizeStoryParty({ version: 3, id: 'party-a', worldId: 'greenhollow', generationVersion: 3, seed: 'seed', leaderSessionId: 'leader', members: [member('leader', 5, roster('leader', 5))], roomId: 'surface', endless: null, updatedAt: 1_000 }, 1_000);
    expect(party.generationVersion).toBe(3);
  ...
  it('benches highest-index AI as humans join and preserves humans after a lower-capacity transfer', () => {
    const party = normalizeStoryParty({ version: 3, id: 'party-a', worldId: 'greenhollow', generationVersion: 3, seed: 'seed', leaderSessionId: 'leader', members: [member('leader', 5, roster('leader', 5)), member('guest', 2, roster('guest', 2), 2)], roomId: 'surface', endless: null, updatedAt: 1_000 }, 1_000);
