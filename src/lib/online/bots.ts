...
export type OnlineBotGenerationInput = {
  seed: string;
...
export function botCpuDifficulty(kp: number, kr: RankedKrScores): CpuDifficulty {
  const averageKr = rankedKrKeys.reduce((sum, key) => sum + kr[key], 0) / rankedKrKeys.length;
  const strength = normalizeKp(kp) + (averageKr - 50) * 8;
...
function normalizeKr(value: Partial<RankedKrScores> | undefined): RankedKrScores {
  const base = emptyRankedKrScores();
  return rankedKrKeys.reduce((next, key) => {
    next[key] = clampScore(value?.[key] ?? base[key]);
    return next;
  }, {} as RankedKrScores);
}
