...
describe('fight engine', () => {
  ...
  it('keeps double-tap up and down sidesteps on the same stage lane after fighters switch sides', () => {
  ...
  it('keeps up and down sidesteps stage-lane aligned on rotated stages after fighters switch sides', () => {
    const rotatedStage: StageDefinition = {
      ...stages[0],
      fightPlane: { center: [4, 0, -2], width: 14, depth: 8, y: 0, rotationY: Math.PI / 2 },
      playableBounds: { shape: 'box', width: 14, depth: 8 }
  ...
  it('does not let a stale sidestep side lock stop lane controls from flipping', () => {
    const sameSide = createMatch(starterCharacters[0], starterCharacters[1], stages[0], 'local2p');
    sameSide.phase = 'fighting';
    sameSide.countdown = 0;
    sameSide.fighters[0].position.x = -1.3;
    sameSide.fighters[1].position.x = 1.3;
    sameSide.fighters[0].controlSideSign = -1;
    sameSide.fighters[0].laneOrbitControlLocked = true;
    sameSide.fighters[0].sidestepDirection = -1;
    const sameSideUp = stepMatch(sameSide, { ...emptyInputFrame(), sidestepUp: true }, emptyInputFrame(), 1 / 60);
    expect(sameSideUp.fighters[0].position.z).toBeLessThan(sameSide.fighters[0].position.z);

    const crossed = createMatch(starterCharacters[0], starterCharacters[1], stages[0], 'local2p');
    crossed.phase = 'fighting';
    crossed.countdown = 0;
    crossed.fighters[0].position.x = 1.3;
    crossed.fighters[1].position.x = -1.3;
    crossed.fighters[0].controlSideSign = 1;
    crossed.fighters[0].laneOrbitControlLocked = true;
    crossed.fighters[0].sidestepDirection = -1;
    const crossedUp = stepMatch(crossed, { ...emptyInputFrame(), sidestepUp: true }, emptyInputFrame(), 1 / 60);
    expect(crossedUp.fighters[0].position.z).toBeLessThan(crossed.fighters[0].position.z);
  });
  ...
  it('reorients continuous lane walking when side crossover changes up/down orbit meaning', () => {
    let match = createMatch(starterCharacters[0], starterCharacters[1], stages[0], 'local2p');
    match.phase = 'fighting';
    match.countdown = 0;
    const laneWalk = emptyInputFrame();
    ...
    expect(flippedDirectionSteps).toBeGreaterThan(0);
  });

  it('keeps up/down lane intent intuitive at the old arena edge while still orbiting', () => {
  ...
  it('reorients repeated down-down taps after crossing sides while preserving orbit', () => {
    let match = createMatch(starterCharacters[0], starterCharacters[1], stages[0], 'local2p');
    match.phase = 'fighting';
    match.countdown = 0;
    let previousAngle = fighterOrbitAngle(match, 0);
  ...
  it('reorients repeated up-up orbit direction at side crossover', () => {
    let match = createMatch(starterCharacters[0], starterCharacters[1], stages[0], 'local2p');
    match.phase = 'fighting';
    match.countdown = 0;
